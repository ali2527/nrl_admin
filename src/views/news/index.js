...
function News() {
  ...
  const resetFilter = () => {
    ...
    getAllNews(paginationConfig.pageNumber, paginationConfig.limit, "", true);
  };
  ...
  const handleLimitChange = (pageSize) => {
    ...
    getAllNews(1, pageSize);
  };

  const handleStatus = async () => {
    try {
      const index = news.findIndex((user) => user._id == selectedUser._id);

      console.log(index)
      const response = await Get(NEWS.toggleStatus + "/" + selectedUser._id , token,{});
      const newUsers = [...news];
  ...
  const getAllNews = async (pageNumber, pageSize, search, reset = false) => {
    setLoading(true);
    try {
      const response = await Get(NEWS.getAllNews, token, {
        page: pageNumber
          ? pageNumber.toString()
      ...
      <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <Button
              ...
              className="mainButton primaryButton"
              onClick={() => navigate("/news/addNewNews")}
            >
              Add News
            </Button>
            ...
            <Input
              ...
              suffix={
                <FaSearch
                  ...
                  onClick={() =>
                    getAllNews(1, paginationConfig.limit, filter.keyword)
                  }
                />
              }
              onPressEnter={(e) =>
