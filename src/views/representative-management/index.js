...
import { REPRESENTATIVE, STATES, UPLOADS_URL } from "../../config/constants";
import { useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";

function RepresentativeManagement() {
  ...
  const handlePageChange = (pageNumber) => {
    ...
    getAllRepresentatives(pageNumber);
  };
  ...
  const resetFilter = () => {
    ...
    getAllRepresentatives(paginationConfig.pageNumber, paginationConfig.limit, "", true);
  };
  ...
  const handleLimitChange = (pageSize) => {
    ...
    getAllRepresentatives(1, pageSize);
  };


  const getAllRepresentatives = async (pageNumber, pageSize, search, reset = false) => {
    setLoading(true);
    try {
      const response = await Get(REPRESENTATIVE.getAllRepresentatives, token, {
        page: pageNumber
          ? pageNumber.toString()
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "position",
      key: "position",
      render: (value, item, index) => value.type,
    },
    {
      title: "Term",
      dataIndex: "reportCard",
      key: "reportCard",
      render: (value, item, index) => value.term,
    },
    {
      title: "Grade",
      dataIndex: "reportCard",
      key: "reportCard",
      render: (value, item, index) => value.grade,
    },
    {
      title: "Action",
      dataIndex: "_id",
      key: "_id",
      render: (item) => (
        <div className="view-link" onClick={() => navigate("/representatives/" + item )}><FaEye style={{fontSize:"16px" , color:"black"}}/></div>
      ),
    },
  ];
  ...
  return (
    <Layout className="configuration">
      ...
      <Row style={{ padding: "10px 20px" }}>
          ...
          >
           <h1 className="pageTitle">Representatives</h1>
          </Col>
          <Col
            xs={24}
            md={12}
          ...
          >
            <Button
              ...
              className="mainButton primaryButton"
              onClick={() => navigate("/representatives/addRepresentative")}
            >
              Add Representative
            </Button>
            ...
            <Input
              ...
              suffix={
                <FaSearch
                  ...
                  onClick={() =>
                    getAllRepresentatives(1, paginationConfig.limit, filter.keyword)
                  }
                />
              }
              onPressEnter={(e) =>
