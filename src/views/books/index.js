...
function BooksManagement() {
  ...
  const handleStatus = async () => {
    try {
      ...
      console.log(index)
      const response = await Get(BOOK.toggleStatus + "/" + selectedBook._id , token,{});
      const newBook = [...books];
      
      console.log(">>>>",newBook[index].isActive)
      console.log(">>>>",selectedBook.isActive)
      newBook[index].status = newBook[index].status == "ACTIVE" ? "INACTIVE" : "ACTIVE";
      setModalOpen(false);
      setBooks(newBook);
    } catch (error) {
  ...
  return (
    <Layout className="configuration">
      ...
      >
        ...
        Do You Want To  {selectedBook?.status == "ACTIVE" ? "Deactivate" : "Activate"} This Book?
        </Typography.Text>
      </Modal>
    </Layout>
  );
