...
function ServicesManagement() {
  ...
  const handleStatus = async () => {
    try {
      const index = services.findIndex((user) => user._id == selectedService._id);

      console.log(index)
      const response = await Get(SERVICE.toggleStatus + "/" + selectedService._id , token,{});
      ...
      },
    {
      ...
      render: (value, item, index) => (
        <Select
          ...
          value={value}
          bordered={false}
          onChange={() => {setModalOpen(true); setSelectedService(services[index])}}
          options={[
            {
              value: "active",
              label: "Active",
