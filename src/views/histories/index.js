...
import { HISTORIES } from "../../config/constants";
...
function Histories() {
  ...
  const resetFilter = () => {
    ...
    getAllHistories(paginationConfig.pageNumber, paginationConfig.limit, "", true);
  };
  ...
  const handleLimitChange = (pageSize) => {
    ...
    getAllHistories(1, pageSize);
  };

  const handleStatus = async () => {
    try {
      const index = histories.findIndex((user) => user._id == selectedUser._id);

      console.log(index)
      const response = await Get(HISTORIES.toggleStatus + "/" + selectedUser._id , token,{});
      const newHistory = [...histories];
      
      console.log(">>>>",newHistory[index].isActive)
      console.log(">>>>",selectedUser.isActive)
      newHistory[index].isActive = !selectedUser.isActive;
      setModalOpen(false);
  ...
  const getAllHistories = async (pageNumber, pageSize, search, reset = false) => {
    ...
    try {
      ...
      if (response?.status) {
        setHistories(response?.data?.docs);
        setPaginationConfig({
            ...
            <Input
              ...
              suffix={
                <FaSearch
                  ...
                  onClick={() =>
                    getAllHistories(1, paginationConfig.limit, filter.keyword)
                  }
                />
              }
              onPressEnter={(e) =>
