...
function Orders() {
  ...
  const resetFilter = () => {
    ...
    getAllOrders(paginationConfig.pageNumber, paginationConfig.limit, "", true);
  };
  ...
  const handleStatus = async () => {
    try {
      const index = orders.findIndex((order) => order._id == selectedOrder._id);
      const response = await Post(ORDERS.updateOrder + selectedOrder._id ,{status:selectedStatus}, token);
      const newOrder = [...orders];
      newOrder[index].status = selectedStatus;
      setModalOpen(false);
      setOrders(newOrder);
    } catch (error) {
  ...
  const getAllOrders = async (pageNumber, pageSize, search, reset = false) => {
    setLoading(true);
    try {
      const response = await Get(ORDERS.getAllOrders, token, {
        page: pageNumber
          ? pageNumber.toString()
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "user",
      key: "user",
      render: (value, item, index) => <>{value?.firstName + " " + value?.lastName  } </>,
    },
    {
      title: "Amount",
    ...
    },
    {
      ...
      render: (item) => (
        ...
        />
        // <div style={{color:'black'}} className="view-link" onClick={() => navigate("/orders/" + item )}>edit</div>
      ),
    },
  ];
  ...
  return (
    <Layout className="configuration">
            ...
            <Input
              ...
              suffix={
                <FaSearch
                  ...
                  onClick={() =>
                    getAllOrders(1, paginationConfig.limit, filter.keyword)
                  }
                />
              }
              onPressEnter={(e) =>
      ...
      >
        ...
        Do You Want To  Change the status to {selectedStatus}
        </Typography.Text>
      </Modal>
    </Layout>
  );
